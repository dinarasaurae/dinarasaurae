...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    fnsh = int(pos[1] // 3 * 3)
    ans = []
    for a in range(strt, strt + 3):
        for b in range(fnsh, fnsh + 3):
            ans.append(grid[a][b])
    return ans
...
def solve(grid: tp.List[tp.List[str]]) -> tp.List[tp.List[str]]:
    ...
    if not pos:
        ...
        if not possible_values:
            return grid
        for increment in possible_values:
            old = grid[pos[0]][pos[1]]
            grid[pos[0]][pos[1]] = increment
            grid = solve(grid)
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for a in range(9):
        if set('123456789') - set(get_row(solution, (0, a))) or set(get_row(solution, (a, 0))):
            return False
    for a in range(3):
        for b in range(3):
            if set('123456789') - set(get_block(solution, (a * 3, a * 3))):
                return False
    return True
